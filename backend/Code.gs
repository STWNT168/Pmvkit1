...
/* =========================================================
   PMV REPORT V1 — Image-aligned 2-column toolkit report
   This module is additive: existing DAILY_DATA is preserved.
   ========================================================= */

function pmvNumber(v) {
  var n = Number(v);
  return Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0;
}

function normalizePmvReport(r) {
  r = JSON.parse(JSON.stringify(r || {}));
  ...
  r.totalPendingKits =
    ...
    r.deliverableKits +
    r.incompleteKits +
    r.improperDetailsKits;

  r.totalPendingArticles =
    r.invalidMobileArticles +
...
function validatePmvReport(r) {
  ...
  if (!r.id) errors.push("Report ID is required.");

  try {
    if (!r.date) errors.push("Date is required.");
    else if (validateDate(r.date) > todayISO()) {
  ...
  if (!r.officeId) {
    ...
    try {
      getOffice(r.officeId);
    } catch (e) {
      errors.push("Office is invalid.");
    }
  }
...
function getPmvReports() {
  return read(SHEETS.PMV_REPORTS);
}

function findPmvReport(spmId, date) {
  return getPmvReports().find(function(r) {
    return String(r.SPM_ID).trim() === String(spmId).trim() &&
      dateOf(r.DATE) === String(date);
  }) || null;
}

function mapPmvReport(r) {
  if (!r) return null;
  ...
  return {
    ...
    deliverableArticles: pmvNumber(r.DELIVERABLE_ARTICLES),
    incompleteKits: pmvNumber(r.INCOMPLETE_KITS),
    incompleteArticles: pmvNumber(r.INCOMPLETE_ARTICLES),
    improperDetailsKits: pmvNumber(r.IMPROPER_DETAILS_KITS),
    improperDetailsArticles: pmvNumber(r.IMPROPER_DETAILS_ARTICLES),
...
function submitPmvReport(record, s) {
  ...
  if (a.role !== ROLES.SPM) {
    return err("Only SPM users can submit the PMV office report.", "FORBIDDEN");
  }

  var r = normalizePmvReport(record);

  if (String(a.user.OFFICE_ID) !== String(r.officeId)) {
    return err("You are not authorized to submit data for this office.", "FORBIDDEN");
  }

  var office = getOffice(r.officeId);
  r.officeName = String(office.OFFICE_NAME || "");
  r.spmId = String(a.user.USER_ID);
  r.spmName = String(a.user.NAME || "");

  var v = validatePmvReport(r);
  if (!v.valid) {
    return {
      success:false,
      code:"VALIDATION",
  ...
  try {
    var existing = findPmvReport(r.spmId, r.date);

    if (existing) {
      return {
        success:false,
        code:"DUPLICATE",
        message:"You have already submitted the PMV report for this date.",
        errors:[]
      };
    }
    ...
    var row = [
      r.id, r.date, r.officeId, r.officeName, r.spmId, r.spmName,
      r.totalPendingKits, r.totalPendingArticles,
      r.invalidMobileKits, r.invalidMobileArticles,
  ...
  return ok({
    recordId:r.id,
    totalPendingKits:r.totalPendingKits,
    totalPendingArticles:r.totalPendingArticles
  }, "PMV report saved successfully.");
}
...
function deleteOwnPmvReport(s, date) {
  ...
  if (a.role !== ROLES.SPM) {
    return err("Only SPM users can delete their PMV report.", "FORBIDDEN");
  }

  var d = validateDate(date);
  if (d !== todayISO()) {
    return err("Only today's PMV report can be deleted.", "DATE_RESTRICTED");
  }
  ...
  try {
    var sh = getSheet(SHEETS.PMV_REPORTS);
    var rows = getPmvReports()
      .filter(function(r) {
        return String(r.SPM_ID).trim() === String(a.user.USER_ID).trim() &&
          dateOf(r.DATE) === d;
      })
      .sort(function(x,y) {
        return Number(y.__row) - Number(x.__row);
      });

    if (!rows.length) {
      return err("No PMV report was found for today.", "NOT_FOUND");
    }
...
function getOwnPmvDashboard(s, date) {
  ...
  if (a.role !== ROLES.SPM) {
    throw new Error("Only SPM users can access the SPM dashboard.");
  }

  var d = validateDate(date);
  var r = findPmvReport(a.user.USER_ID, d);

  return ok({
    date:d,
    report:mapPmvReport(r)
  });
}
...
function getAdminPmvDashboard(s, date) {
  ...
  if (a.role !== ROLES.DPS && a.role !== ROLES.ADMIN) {
    throw new Error("Only DPS/Admin users can access the consolidated PMV dashboard.");
  }

  var d = validateDate(date);
  var users = read(SHEETS.USER_MASTER).filter(function(u) {
    return active(u.ACTIVE) && normRole(u.ROLE) === ROLES.SPM;
  });

  // One logical report per SPM/date; newest physical row wins.
  var reportMap = {};
  getPmvReports().forEach(function(r) {
    if (dateOf(r.DATE) !== d) return;
    var key = String(r.SPM_ID).trim();
    if (!reportMap[key] || Number(r.__row) > Number(reportMap[key].__row)) {
      reportMap[key] = r;
    }
  });
  ...
  read(SHEETS.OFFICE_MASTER).filter(function(o) {
    return active(o.ACTIVE);
  }).forEach(function(o) {
    var oid = String(o.OFFICE_ID);
    officeMap[oid] = {
      officeId:oid,
      officeName:String(o.OFFICE_NAME || ""),
      totalSpms:0,
  ...
  Object.keys(reportMap).forEach(function(uid) {
    var r = reportMap[uid];
    var oid = String(r.OFFICE_ID);
    ...
    var o = officeMap[oid];
    o.totalPendingKits += pmvNumber(r.TOTAL_PENDING_KITS);
    o.totalPendingArticles += pmvNumber(r.TOTAL_PENDING_ARTICLES);
    o.invalidMobileKits += pmvNumber(r.INVALID_MOBILE_KITS);
    o.invalidMobileArticles += pmvNumber(r.INVALID_MOBILE_ARTICLES);
    o.deliverableKits += pmvNumber(r.DELIVERABLE_KITS);
    o.deliverableArticles += pmvNumber(r.DELIVERABLE_ARTICLES);
    o.incompleteKits += pmvNumber(r.INCOMPLETE_KITS);
  ...
  offices.forEach(function(o) {
    kpis.totalPendingKits += o.totalPendingKits;
    kpis.totalPendingArticles += o.totalPendingArticles;
    kpis.invalidMobileKits += o.invalidMobileKits;
    kpis.invalidMobileArticles += o.invalidMobileArticles;
    kpis.deliverableKits += o.deliverableKits;
    kpis.deliverableArticles += o.deliverableArticles;
    kpis.incompleteKits += o.incompleteKits;
  ...
  var updated = users.filter(function(u) {
    return !!reportMap[String(u.USER_ID)];
  }).length;
  ...
  return ok({
    ...
    activeSpms:total,
    spmsUpdated:updated,
    spmsPending:total - updated,
    completionPercentage:total ? round(updated / total * 100) : 0,
    officeWise:offices,
